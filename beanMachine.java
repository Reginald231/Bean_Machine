...
public class beanMachine extends Application{
   ...
   public static PathTransition[] transits = new PathTransition[10];//animations for balls
   public static Polyline[] paths = new Polyline[10];//Lines for each circle and animation
   public static int slotCount[] = new int[8];//Array for recording the number of balls in the slots.
   public static int i = 1;//a default value required for "playRemainingTransits()"
   ...
   public void start(Stage primaryStage){
      ...
      //Creating the left rim of the outline
   ...
   //Designing The Outline of The Grid: Begin._________________________________________________________
      ...
      begin.setLayoutX(WSW / 2);
      begin.setLayoutY(WSH / 2);

      //Welcome text when starting the program.
      Text text = new Text((500 / 2), (600 / 2) + 50, "Welcome to the Bean Machine Program." +
         "\nClick the button below to begin.");

      pane2.setBottom(begin);
      pane2.setAlignment(begin, Pos.BOTTOM_CENTER);
      pane2.setCenter(text);
      pane2.setMargin(begin, new Insets(0, 200,200,200));
      Scene scene2 = new Scene(pane2, WSW, WSH);
      ...
      begin.setOnAction(e -> primaryStage.setScene(scene));

      primaryStage.setScene(scene2);//Self-explanatory. Setting the scene.
      primaryStage.setTitle("Bean Machine");//Setting the title of the stage
      primaryStage.show();//Displaying the stage.
   }

   public static void directionMaker() throws NullPointerException{//creates new coordinates for a line after calling another method for
   //randomly generated directions.

      turns = new double[32];

      //The 4 values below are the default values for the first 2 points of each line.
      ...
      for (int i = 4; i < turns.length; i = i + 4){
         ...
         if(turns[i] == 0){
            throw new NullPointerException("Exception: A value was misplaced in the turns array. " +
               "Program cannot run properly. Exiting program...");
         }
      }

      /*Uncomment to find out the x and y values for each point of the line.
   ...
   public static int checkSlots(double xValue) throws NullPointerException{
      ...
      for (int i = 0; i < slotPoints.length; i++){
         ...
         if(slotPoints[i] == 0)
            throw new NullPointerException("Exception: There is a missing value within the slotPoints array." +
   ...
   public static char[] giveDirections(){
      ...
      /*Uncomment to display the path of each line.
   ...
   public void playRemainingTransits()throws ArithmeticException{//Use of recursion in this method.

      if (i == 0)
         throw new ArithmeticException("Exception: to prevent the playing of the first transition." +
            " 'i' must must not equal 0.");
